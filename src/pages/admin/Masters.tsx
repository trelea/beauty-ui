...
export const Masters: React.FC = () => {
  ...
  React.useEffect(() => setPage({ page: page.get("page") as string }), [page]);
      ...
      <div className="px-8 md:px-11 xl:px-40 2xl:px-56 pt-36 min-h-screen flex flex-col gap-4">
        <div className="flex gap-10 items-center relative">
          ...
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" className="rounded-full shadow-xl">
                {t("adminMasters.createBtn")}
              </Button>
            </SheetTrigger>
            <SheetContent>
              <SheetHeader>
                <SheetTitle>{t("adminMasters.createBtn")}</SheetTitle>
              </SheetHeader>
        ...
        {(isLoading || isFetching) && (
          <div className="flex flex-col justify-center items-center w-full flex-grow">
            ...
            <h1 className="text-lg font-medium">{t("adminMasters.load")}</h1>
          </div>
        )}
